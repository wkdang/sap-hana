...
resource "azurerm_resource_group" "hana_rg" {
  name     = "${local.rg_name}"
  location = "${local.hana_regions[0]}"
}

# NETWORK SECURITY GROUP ==============================================
resource "azurerm_network_security_group" "hana_nsg" {
  name                = "${local.resource_prefix}hana-nsg"
  location            = "${local.hana_regions[0]}"
...
module "hana_instance_1" {
  ...
  hana_disk_sizes_data   = "${local.hana_disk_sizes_data}"
  hana_disk_sizes_log    = "${local.hana_disk_sizes_log}"
  hana_disk_sizes_shared = "${local.hana_disk_sizes_shared}"
  hana_address_space     = "${local.hana_address_spaces[0]}"
  hana_subnet            = "${local.hana_subnets[0]}"
  hana_enable_public_ip  = "${local.hana_enable_public_ip}"
  hana_region            = "${local.hana_regions[0]}"
  hana_nsg_id            = "${azurerm_network_security_group.hana_nsg.id}"
  hana_instance_name     = "${replace(replace(local.label_instance, "[type]", "db"), "[site]", local.hana_enable_ha ? "-primary" : "")}"
}
